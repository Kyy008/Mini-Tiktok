...
- 支持 HTTP Range 分段播放，命中 Range 时返回 `206 Partial Content`。
- 支持浏览器 `<video>` 标签播放、拖动进度条和按需加载。
- 播放链路可参考“服务端流式响应/大规模数据传输/视频流”的设计思想，在浏览器场景下通过 HTTP Range 落地，而不是 gRPC。
- 当前阶段不在应用层实现限流；后续如需保护播放流量，可结合令牌桶或并发控制增强。
- 删除的视频返回 404。
